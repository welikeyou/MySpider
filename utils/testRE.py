...
def parse_local_data(data):
   ...
   #
    ...
    try:
        phoneNum1 = re.search(r'(甲方联系方式|甲方电话|采购单位电话|采购单位联系方式|采购单位联系电话)(：)*((\d{3}-\d{8})|1\d{10})', data)
        if phoneNum1:
            parseData.jia_contact_way = phoneNum1.group(3)
            time += 1

        phoneNum2 = re.search(r'(项目联系方式|项目联系电话)(：)*((\d{3}-\d{8})|1\d{10})', data)
        if phoneNum2:
            parseData.yi_contact_way = phoneNum2.group(3)
            time += 1

        span = 0
        while time < 3 and (phoneNum1 is None or phoneNum2 is None):
            time += 1
            # search只能搜到第一个匹配的字符串，循环搜索无用，需修改
            phoneNum = re.search(r'(电话|联系方式)(：)*((\d{3}-\d{8})|1\d{10})', data[span:len(data)])
            startIndex = max(phoneNum.span()[0] - 20, 0)  # 先往前取50个字
            endIndex = phoneNum.span()[1]
            span = endIndex
            dis_num = distinguish_jia_yi(data, startIndex, endIndex, 20, 10)
            if dis_num > 0:
    ...
    try:
        ...
        if jia_name:
            ...
            time += 1
        yi_name = re.search(r'(乙方名称|代理机构名称|供应商名称|供应商单位名称|中标人)(：)*((([^，,；。])*?)公司)', data)
        if yi_name:
            parseData.yi_name = yi_name.group(3)
            time += 1

        while time < 3 and (jia_name is None or yi_name is None):
            time += 1
    ...
    try:
        ...
        time = 0
        span = 0
        jia_linkman = re.search(r'(甲方联系人|采购单位联系方式)(：)*([\u4E00-\u9FA5]{2,3})', data)
        if jia_linkman:
            parseData.jia_linkman = jia_linkman.group(3)
            time += 1
        yi_linkman = re.search(r'(乙方联系人|项目联系方式)(：)*([\u4E00-\u9FA5]{2,3})', data)
        if yi_linkman:
            parseData.yi_linkman = yi_linkman.group(3)
            time += 1

        while time < 3 and (jia_linkman is None or yi_linkman is None):
            time += 1
            ...
            if dis_num > 0:
                ...
                parseData.yi_linkman = linkman.group()
    except AttributeError:
        linkman = "null"
    ...
    try:
        ...
        try:
            parseData.content = re.match(r'(.*?)(中标公告|结果公示)', data).group()
        except AttributeError:
    ...
    try:
        parseData.time = re.search(r'(时间|日期)[^、。，；1-9]*((\d+-\d+-\d+)|(\d+年\d+月\d+日)|(\d+/\d+/\d+))', data).group(2)
    except AttributeError:
        try:
            parseData.time = re.search(r'(\d+-\d+-\d+)|(\d+年\d+月\d+日)', data).group()
        except AttributeError:
            parseData.time = "null"
    if parseData.time.find('-') == -1:
...
if __name__ == "__main__":
    ...
分院区数据中心改造及升级项目 项目编号：项目联系方式：项目联系人：王科长项目联系电话：027-83662896采购单位联系方式：采购单位：华中科技大学同济医学院附属同济医院地址：武汉市解放大道1095号联系方式：王科长；027-83662896代理机构联系方式：代理机构：湖北省成套招标股份有限公司代理机构联系人：刘李鹏、胡小康； 027-87816666-8206、8209代理机构地址： 武昌区东湖西路平安财富中心B座7-10楼一、采购项目的名称、数量、简要规格描述或项目基本概况介绍：包号内容采购需求（简要）预算金额/最高限价质量标准交货（安装）期/服务期质保期/缺陷责任期备注数据中心改造及升级机房管线防结露保温，接缝处严密；楼板保温；更换扣板；空气循环区域防尘处理；使室内保持正压使尘埃不易进入室内；机房内所有空洞及衔接处均密封；设立缓冲空间；完整的业务数据库迁移解决方案。210万元合格合同签订后60日历日内供货并安装完毕，验收合格数据中心主机：5年，其余部分：2年二、投标人的资格要求：1. 符合《中华人民共和国政府采购法》第二十二条规定的条件；2. 其他资格要求：提供投标人参加政府采购活动前3年内在经营活动中没有重大违法记录的书面声明（格式要求详见第六章相关格式）3. 未被列入“信用中国”网站( www.creditchina.gov.cn)失信被执行人、重大税收违法案件当事人名单、政府采购严重违法失信行为记录名单和“中国政府采购”网站（ www.ccgp.gov.cn）政府采购严重违法失信行为记录名单（以投标截止时间前查询结果为准）；4. 本次招标不接受联合体投标。联合体投标的，应满足下列要求：本项目不适用。5. 如国家法律法规对市场准入有要求的还应符合相关规定。三、招标文件的发售时间及地点等：预算金额：210.0 万元（人民币）时间：2019年07月12日 08:30 至 2019年07月18日 16:30(双休日及法定节假日除外)地点：湖北省成套招标股份有限公司服务大厅 湖北省武汉市武昌区东湖西路特2号平安财富中心B座七楼。招标文件售价：￥300.0 元，本公告包含的招标文件售价总和招标文件获取方式：供应商获取招标文件时须携带以下资料：供应商法定代表人凭法定代表人身份证明书（原件）或委托代理人凭法定代表人授权书（原件）、本人身份证（原件）。 供应商需开具发票应提供以下内容：供应商单位名称、纳税人识别号或统一社会信用代码、地址、电话、开户银行及账号。招标文件获取时间：2019年7月12日起至2019年7月18日每天上午8:30～12:00 、下午14:00～16:30 （节假日除外）。四、投标截止时间：2019年08月01日 09:30五、开标时间：2019年08月01日 09:30六、开标地点：湖北省成套招标股份有限公司10楼1009号会议室七、其它补充事宜八、采购项目需要落实的政府采购政策：中小企业、节能环保、监狱企业、促进残疾人就业湖北省成套招标股份有限公司数据中心 Copyright  2016 |  job592.Com | All rights reserved | 招标信息预警网 | 版权所有 首页工资面试评价工资待遇求职招聘我的职业圈意见反馈 "
    ...
    data8="当前位置：首页 » 政采公告 » 地方公告 » 中标公告武汉大学中南医院超声骨动力系统采购项目（三次）中标结果公告2019年07月12日 17:41 来源：中国政府采购网 【打印】 【显示公告正文】【显示公告概要】公告概要：公告信息：采购项目名称武汉大学中南医院超声骨动力系统采购项目品目货物/专用设备/医疗设备/医用超声波仪器及设备采购单位武汉大学中南医院行政区域武汉市公告时间2019年07月12日  17:41本项目招标公告日期2019年06月21日中标日期2019年07月12日评审专家名单郑富强、徐文胜、姚迅、秦冬梅、胡超总中标金额￥150.000000 万元（人民币）联系人及联系方式：项目联系人刘工项目联系电话027-87272702 采购单位武汉大学中南医院采购单位地址武汉市武昌区东湖路169号采购单位联系方式邢老师 027-67812503代理机构名称湖北中天招标有限公司代理机构地址湖北省武汉市武昌区中北路109号中铁1818中心10楼代理机构联系方式刘工027-87272702湖北中天招标有限公司受武汉大学中南医院的委托，就“武汉大学中南医院超声骨动力系统采购项目”项目（项目编号：ZB0205-1904-ZCHW0021）组织采购，评标工作已经结束，中标结果如下： 一、项目信息项目编号：ZB0205-1904-ZCHW0021项目名称：武汉大学中南医院超声骨动力系统采购项目项目联系人：刘工联系方式：027-87272702  二、采购单位信息采购单位名称：武汉大学中南医院采购单位地址：武汉市武昌区东湖路169号采购单位联系方式：邢老师 027-67812503 三、项目用途、简要技术要求及合同履行日期：采购超声骨动力系统1套（技术要求详见招标文件第三章项目技术、服务及商务要求（已完成进口论证）交货期：合同签订后两个月内，允许提前交货质保期：产品验收合格后至少12个月 四、采购代理机构信息采购代理机构全称：湖北中天招标有限公司采购代理机构地址：湖北省武汉市武昌区中北路109号中铁1818中心10楼采购代理机构联系方式：刘工027-87272702 五、中标信息招标公告日期：2019年06月21日中标日期：2019年07月12日总中标金额：150.0 万元（人民币）中标供应商名称、联系地址及中标金额：中标单位名称：国药控股湖北医疗器械有限公司中标单位地址：武汉市东湖新技术开发区高新大道666号CRO办公区A19栋10楼B区中标金额：人民币壹佰伍拾万元整（¥1,500,000.00）本项目招标代理费总金额：0.0 万元（人民币）本项目招标代理费收费标准：根据国家发展与改革委员会办公厅发改办价格[2003]857号文的规定，经与采购人协商，由中标人按国家发展计划委员会计价格【2011】534号文及原国家计委《招标代理服务收费管理暂行办法》（【2002】1980号）规定的标准向招标代理机构支付代理服务费。 评审专家名单：郑富强、徐文胜、姚迅、秦冬梅、胡超 中标标的名称、规格型号、数量、单价、服务要求：中标单位名称：国药控股湖北医疗器械有限公司中标单位地址：武汉市东湖新技术开发区高新大道666号CRO办公区A19栋10楼B区中标金额：人民币壹佰伍拾万元整（¥1,500,000.00）数量：1套交货期：合同签订后30天内质保期：产品验收合格后5年 六、其它补充事宜1、公告媒体：中国政府采购网、中国招标投标公共服务平台、湖北中天招标有限公司网站2、中标公告期限：1个工作日3、各有关当事人对中标结果有异议的，可以在中标公告发布之日起七个工作日内以书面形式向采购人及采购代理机构提出质疑，逾期将不再受理。  相关公告"
